...
//   - 빨강(.error)   : 아직 교정 못 한 평가 대상(remaining)이 이번에도 틀림
//   - 파랑(.success) : 원래 틀렸던 음절을 맞춤(이미 교정해 잠긴 음절 포함)
//   - 일반(.normal)  : 평가 대상이 아님(원래도 맞았던 음절 등) — 틀려도 빨강 아님
//
//  성공 판정은 "평가 대상(remaining)이 모두 교정되면(remaining - corrected = ∅) 성공"이다.
//  remaining 음절이 이번 시도에서 누락(deletion)되면 corrected 에 들지 않아 성공을 막는다.
//  반면 원래 맞았던(=평가 대상 아닌) 음절은 틀리거나 누락돼도 성공에 영향을 주지 않는다
//  — 모델 오인식 때문에 전체 성공이 무한정 지연되지 않게 하기 위함이다(이슈 #117 후속).
//
