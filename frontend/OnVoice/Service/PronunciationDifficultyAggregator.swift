...
enum PronunciationDifficultyAggregator {
    ...
    private static func buildResult(
        ...
        return PronunciationDifficultyResult(
            ...
            accentColorHex: info.accentColorHex,
            imageName: "error_img_1",
            errorCount: count
        )
    }

    /// 카테고리별 표시 텍스트와 강조 색상. 색상은 slot 기준
    /// (초성=#FFA0A0, 중성=#B2B8FF, 종성=#FFF79E, 탈락=#FFA0A0).
    /// 텍스트는 잠정 카피 — UI 확정 후 별도 이슈에서 다듬는다.
    private static func displayInfo(
        for category: PronunciationErrorCategory
    ) -> (subtitle: String, practiceTitle: String, guideText: String, accentColorHex: String) {
        switch category {
        case .vowelError:
            return (
                "모음을 정확히 발음해보세요",
