...
struct RecordingAnalysisView: View {
    ...
    private var currentSentence: SentenceComparison? {
        guard currentSentenceIndex < recognizer.errorSentences.count else { return nil }
        return recognizer.errorSentences[currentSentenceIndex]
    }
    
    // 마지막 문장인지 확인
    private var isLastSentence: Bool {
        return currentSentenceIndex == recognizer.errorSentences.count - 1
    }
    
    // 연습 완료 여부 (80% 이상 정확도 달성)
    private var hasReachedTarget: Bool {
        return speechAnalyzer.hasReachedTarget
    }
    
    // 4회 연습 완료 여부
    private var hasCompletedFourAttempts: Bool {
        return speechAnalyzer.practiceCount >= 4
    }
    ...
    private var canProceedToNext: Bool {
        ...
        return hasReachedTarget || hasCompletedFourAttempts
    }
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 16) {
                        if let sentence = currentSentence {
                            ...
                            // 연습 결과 (있는 경우)
                            if speechAnalyzer.practiceCount > 0 {
                                practiceResultView
                            }
                        }
                    }
...
//        .padding(.bottom, 20)
    ...
    private var practiceStatusView: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack {
                Text("연습 횟수: \(speechAnalyzer.practiceCount)/4")
                    .font(.Pretendard.Medium.size14)
                    .foregroundColor(.white)
                
                Spacer()
                
                if speechAnalyzer.hasReachedTarget {
                    Text("목표 달성! 🎉")
                        .font(.Pretendard.Bold.size14)
                        .foregroundColor(.point)
                } else if speechAnalyzer.practiceCount >= 4 {
                    Text("4회 완료 - 계속 연습 가능")
                        .font(.Pretendard.Medium.size14)
    ...
    private var practiceResultView: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 8) {
                Text("인식된 발음: \(speechAnalyzer.recognizedText)")
                    .font(.Pretendard.Regular.size14)
                    .foregroundColor(.white)
                
                Text("정확도: \(Int(speechAnalyzer.currentAccuracy))%")
                    .font(.Pretendard.Medium.size14)
                    .foregroundColor(speechAnalyzer.currentAccuracy >= 80 ? .point : .red)
            }
            .padding(12)
            .background(Color.suGray7)
            .clipShape(RoundedRectangle(cornerRadius: 8))
        ...
        VStack(spacing: 16) {
            ...
            Button(action: {
                if !speechAnalyzer.isRecording {
                    Task {
                        if let sentence = currentSentence {
                            await speechAnalyzer.startPronunciationPractice(
                ...
                Image(systemName: speechAnalyzer.isRecording ? "stop.circle.fill" : "mic.circle.fill")
                    .font(.system(size: 60))
                    .foregroundColor(speechAnalyzer.isRecording ? .red : (canRecord ? .point : .suGray6))
            }
            .disabled(!canRecord && !speechAnalyzer.isRecording)
    ...
    private func moveToNextSentence() {
        if currentSentenceIndex < recognizer.errorSentences.count - 1 {
            currentSentenceIndex += 1
            speechAnalyzer.resetPractice()
        }
    }
}
